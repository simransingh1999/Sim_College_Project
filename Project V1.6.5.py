...
def buildSystem():
    ...
    def createCapUtil(frame):
        ...
        for meat in mType:
            ...
            maxCapacity = ttk.Label(frame, text = meat[3]).grid(row=i, column=7, ipady=5)
            mCap = meat[3]
            capacityUtil = AvoidZeroDiv(cCap, mCap)
            capacityUtil = round(capacityUtil, 1)
            capacityUtilisationLabel = ttk.Label(frame, text = capacityUtil).grid(row=i, column=9, ipady=5)
            i += 2
