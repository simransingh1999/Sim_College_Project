...
c.execute("CREATE TABLE Products (Product_Number INTEGER PRIMARY KEY, Price Per lb FLOAT, Meat_Type TEXT, Meat_Part TEXT, Costs Per lb FLOAT, Quantity 'KG' FLOAT)")
...
c.execute("CREATE TABLE Sales (Sales_ID INTEGER PRIMARY KEY, Date_Time datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, Meat_Type TEXT, Meat_Part TEXT, Meat_Weight 'lb' FLOAT, Total FLOAT)")
...
def buildSystem():
    ...
    def stockAdd(frame):
        ...
        def addButtonFunction():
            ...
            mW = weightEntry.get()
            mC = costEntry.get()
            mQ = individualMeatQuantity()
            totalMeatCost = individualMeatCost()
            ...
            if mT == 'Turkey':
                ...
                if confirm == 'yes':
                    ...
                    mC = float(mC)
                    mC = mC / 2.2
                    oldMeatCost = totalMeatCost * mQ
                    mQ = mQ + mW
                    total = (totalMeatCost + mC) / mQ
                    total = round(total, 2)
                    conn = sqlite3.connect('Punjab Meat Shop.db') 
                    c = conn.cursor()
                    c.execute("UPDATE Products SET Costs=?, Quantity=? WHERE Meat_Type = ? AND Meat_Part = ?", (total, mQ, mType, mPart))
                    conn.commit()
                    conn.close()
                    messagebox.showinfo("Success!", "Your order was successfully added.")
                    buildStock()
