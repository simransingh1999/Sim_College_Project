...
c.execute("CREATE TABLE Products (Product_Number INTEGER PRIMARY KEY, Price Per KG FLOAT, Meat_Type TEXT, Meat_Part TEXT, Costs Per KG FLOAT, Quantity KG FLOAT, Maximum_Quantity KG INTEGER)")

#Chicken
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (0.70, 'Chicken', 'Whole',  0, 0, 70)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (1.16, 'Chicken', 'Fillet', 0, 0, 20)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (0.68, 'Chicken', 'Drum Sticks', 0, 0, 140)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (0.45, 'Chicken', 'Legs', 0, 0, 100)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (1.11, 'Chicken', 'Thigh Meat', 0, 0, 40)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (0.90, 'Chicken', 'Wings', 0, 0, 20)")

#Lamb
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (2.13, 'Lamb', 'Legs', 0, 0, 12)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (1.90, 'Lamb', 'Shoulder', 0, 0, 12)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (1.81, 'Lamb', 'Chops', 0, 0, 8)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (1.81, 'Lamb', 'Back', 0, 0, 8)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (1.81, 'Lamb', 'Mince', 0, 0, 50)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (0.90, 'Lamb', 'Ribs', 0, 0, 20)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (0.90, 'Lamb', 'Neck', 0, 0, 8)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (1.95, 'New Zealand Lamb', 'Leg', 0, 0, 10)")

#Specialties
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity) VALUES (0, 'Turkey', 'Whole', 0, 0)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (1.13, 'Pork', 'Leg', 0, 0, 10)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (1.63, 'Masala Fish', 'N/A', 0, 0, 15)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (1.01, 'Marinated Chicken', 'Wings', 0, 0, 20)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (0.82, 'Marinated Chicken', 'Legs', 0, 0, 100)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (0.89, 'Marinated Chicken', 'Drum Sticks', 0, 0, 140)")
c.execute("INSERT INTO Products (Price, Meat_Type, Meat_Part, Costs, Quantity, Maximum_Quantity) VALUES (1.76, 'Goat', 'Meat', 0, 0, 15)")

"Sales Table"

c.execute("CREATE TABLE Sales (Sales_ID INTEGER PRIMARY KEY, Date TEXT, Time TEXT, Meat_Type TEXT, Meat_Part TEXT, Meat_Weight 'KG' FLOAT, Total FLOAT)")
...
c.execute("CREATE TABLE Cooler (Check_Number INTEGER PRIMARY KEY, Date TEXT, Time TEXT, Water_Bucket FLOAT, Temperature FLOAT, Rating INTEGER, Time_Taken INTEGER, Capacity_Utilisation FLOAT)")
...
def buildSystem():
    ...
    def createLogo(frame):
        ...
        chick = ttk.Label(frame, text="Chicken (price per LB)", font='Helvetica 13 bold')
        ...
        lamb = ttk.Label(frame, text="Lamb (price per LB)", font='Helvetica 13 bold')
        ...
        special = ttk.Label(frame, text="Specialties (price per LB)", font='Helvetica 13 bold')
    ...
    def reorderQuantityLevel():
        conn = sqlite3.connect("Punjab Meat Shop.db")
        c = conn.cursor()

        c.execute("SELECT Meat_Type, Meat_Part, Quantity, Maximum_Quantity FROM Products")
        ...
        for meat in mType:
            currentCapacityUtil = AvoidZeroDiv(meat[2], meat[3])
            currentCapacityUtil = round(currentCapacityUtil, 1)
            if currentCapacityUtil <= 25:
                mLow = meat[0], meat[1]
                lowQuantity += 1

        return lowQuantity

    def coolerCapacityUtilisation():
        maxCoolerCap = 600

        conn = sqlite3.connect("Punjab Meat Shop.db")
        c = conn.cursor()

        c.execute("SELECT Quantity FROM Products")

        currentQuantity = c.fetchall()

        conn.close()

        currentCap = 0
        
        for quantity in currentQuantity:
            currentCap = currentCap + quantity[0]

        coolerCap = AvoidZeroDiv(currentCap, maxCoolerCap)
        coolerCap = round(coolerCap, 1)

        return coolerCap
            
    def createHome(frame):
        def reorderButton():
            changePage("reorderButton")
        def capUtil():
            changePage("capUtil")
        ...
        noOfSalesLabel = ttk.Label(frame, text=numberOfSales())
        noOfSalesLabel.grid(column=3, row=5, sticky=(N))
        reorder = ttk.Button(frame, text="Reorder Quantity Level: ", command=reorderButton)
        reorder.grid(column=5, row=2, sticky=(N))
        reorderLabel = ttk.Label(frame, text=reorderQuantityLevel())
        reorderLabel.grid(column=7, row=2, sticky=(N))
        capacityUtil = ttk.Button(frame, text="Capacity Utilisation (%): ", command=capUtil)
        capacityUtil.grid(column=5, row=4, sticky=(N))
        capacityUtilLabel = ttk.Label(frame, text=coolerCapacityUtilisation())
        capacityUtilLabel.grid(column=7, row=4, sticky=(N))
    ...
    def reorderQuantityButton(frame):

        treeview = ttk.Treeview(frame)
        treeview.grid(column=0, row=0, sticky=(N,E,S,W))
        treeview.config(columns = ('Meat Type', 'Meat Part', 'Current Capacity', 'Maximum Capacity', 'Capacity Utilisation'))
        treeview.column('#0', width=0, anchor='center')
        treeview.column('Meat Type', width=150, anchor='center')
        treeview.heading('Meat Type', text='Meat Type')
        treeview.column('Meat Part', width=150, anchor='center')
        treeview.heading('Meat Part', text='Meat Part')
        treeview.column('Current Capacity', width=100, anchor='center')
        treeview.heading('Current Capacity', text='Current Capacity')
        treeview.column('Maximum Capacity', width=150, anchor='center')
        treeview.heading('Maximum Capacity', text='Maximum Capacity')
        treeview.column('Capacity Utilisation', width=150, anchor='center')
        treeview.heading('Capacity Utilisation', text='Capacity Utilisation')

        vsb = ttk.Scrollbar(frame, orient="vertical", command=treeview.yview)
        
        vsb.grid(column=10, row=0, sticky='ns', in_=frame)
        ...
        for meat in mType:
            ...
            if currentCapacityUtil <= 25:
                ...
                quantity = meat[2]
                maxQuantity = meat[3]
                capUtil = AvoidZeroDiv(quantity, maxQuantity)
                capUtil = round(capUtil, 1)
                meatType = meatType.replace(" ","\xa0")
                treeview.insert('', i, values=(meatType, meatPart, quantity, maxQuantity, capUtil))
        ...
        def addButtonFunction():
            ...
            if mT == 'Turkey':
                ...
                if confirm == 'yes':
                    ...
                    conn = sqlite3.connect('Punjab Meat Shop.db') 
                    c = conn.cursor()
                    c.execute("INSERT INTO Sales (Date, Time, Meat_Type, Meat_Part, Meat_Weight, Total) VALUES (?, ?, ?, ?, ?, ?)", (nowDate, nowTime, mT, mP, mW, total))
                    c.execute("UPDATE Products SET Quantity=? WHERE Meat_Type = ? AND Meat_Part = ?", (newMQ, mT, mP))
                    conn.commit()
                    conn.close()
    ...
    def orderDelete(frame):
        date = ttk.Label(frame, text="Date", font='Helvetica 13 bold')
        date.grid(column=1, row=1, sticky=(N), padx=5)
        total = ttk.Label(frame, text="Total", font='Helvetica 13 bold')
        total.grid(column=3, row=1, sticky=(N), padx=5)
        select = ttk.Label(frame, text="Select", font='Helvetica 13 bold')
        select.grid(column=5, row=1, sticky=(N), padx=5)
        ...
        c.execute("SELECT Date, Total FROM Sales WHERE Date = ?", (nowDate,))

        salesResult = c.fetchall()

        dateResult = [ ]
        
        for sale in salesResult:
            dateResult.append(sale)
        ...
        for date in dateResult:
            dateLabel = ttk.Label(frame, text=date[0])
            dateLabel.grid(column=1, row=i, sticky=(N))
            totalLabel = ttk.Label(frame, text=date[1])
            totalLabel.grid(column=3, row=i, sticky=(N))
            selectButton = ttk.Button(frame, text="Select")
            selectButton.grid(column=5, row=i, sticky=(N))
            i += 1
    ...
    def costTotal():
        conn = sqlite3.connect("Punjab Meat Shop.db")
        c = conn.cursor()
        c.execute("SELECT Costs, Quantity FROM Products")
        ...
        for row in total:
            cost = row[0]
            quantity = row[1]
            runningTotal = cost * quantity
            meatCost = meatCost + runningTotal
    ...
    def createFinances(frame):
        ...
        costsLabel = ttk.Label(frame, text=costTotal())
        costsLabel.grid(column=3, row=5, sticky=(N))
    ...
    def createCooler(frame):
        ...
        capUtilLabel = ttk.Label(frame, text=coolerCapacityUtilisation())
        capUtilLabel.grid(column=3, row=1, sticky=(N))
        ...
        def addButtonFunction():
            ...
            if checkNum(getWater) == True and checkNum(getTemp) == True and checkNum(getRating) == True and checkNum(getTime) == True:
                ...
                if confirm == 'yes':
                    ...
                    conn = sqlite3.connect('Punjab Meat Shop.db') 
                    c = conn.cursor()
                    c.execute("INSERT INTO Cooler (Date, Time, Water_Bucket, Temperature, Rating, Time_Taken) VALUES (?, ?, ?, ?, ?, ?)", (nowDate, nowTime, cW, cTe, cR, cTi))
                    conn.commit()
                    conn.close()
        ...
        def addButtonFunction():
            ...
            if checkNum(mW) == True and checkNum(mPrice) == True:
                ...
                if confirm == 'yes':
                    ...
                    conn = sqlite3.connect('Punjab Meat Shop.db') 
                    c = conn.cursor()
                    c.execute("INSERT INTO Sales (Date, Time, Meat_Type, Meat_Part, Meat_Weight, Total) VALUES (?, ?, ?, ?, ?, ?)", (nowDate, nowTime, mT, mP, mW, mPrice))
                    conn.commit()
                    conn.close()
                    messagebox.showinfo("Success!", "Your order was successfully added.")
                    buildPromotions()
    ...
    def promotionStockAdd(frame):
        ...
        def addButtonFunction():
            ...
            if checkNum(mW) == True and checkNum(mC) == True: 
                confirm = messagebox.askquestion( "Are you sure you want to add this order?", order, icon='warning')
                if confirm == 'yes':
                    mType = "Turkey"
                    mPart = "Whole"
    ...
    def changePage(page):
        ...
        if page == "Home":
            createTitle("Home")
            createHome(content)
        elif page == "reorderButton":
            createTitle("Reorder Quantity Levels")
            reorderQuantityButton(content)
        elif page == "Orders":
            createTitle("Orders")
            ...
            createOrder(content)
        elif page == "deleteOrder":
            createTitle("Delete Order")
            orderDelete(content)
        elif page == "addStock":
