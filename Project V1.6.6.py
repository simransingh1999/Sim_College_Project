...
def buildSystem():
    ...
    def reorderQuantityButton(frame):
        meatType = ttk.Label(frame, text="Meat Type", font='Helvetica 13 bold')
        meatType.grid(column=1, row=0, sticky=(N), ipady=15)

        meatPart = ttk.Label(frame, text="Meat Part", font='Helvetica 13 bold')
        meatPart.grid(column=3, row=0, sticky=(N), ipady=15)

        currentCapacity = ttk.Label(frame, text="Current Capacity (KG)", font='Helvetica 13 bold')
        currentCapacity.grid(column=5, row=0, sticky=(N), ipady=15)

        maximumCapacity = ttk.Label(frame, text="Maximum Capacity (KG)", font='Helvetica 13 bold')
        maximumCapacity.grid(column=7, row=0, sticky=(N), ipady=15)

        capacityUtilisation = ttk.Label(frame, text="Capacity Utilisation (%)", font='Helvetica 13 bold')
        capacityUtilisation.grid(column=9, row=0, sticky=(N), ipady=15)
        ...
        for meat in mType:
            ...
            currentCapacityUtil = round(currentCapacityUtil, 1)
            if currentCapacityUtil <= 25:
                meatType = ttk.Label(frame, text = meat[0]).grid(row=i,column=1, ipady=5)
                meatPart = ttk.Label(frame, text = meat[1]).grid(row=i,column=3, ipady=5)
                currentCapacity = ttk.Label(frame, text = meat[2]).grid(row=i, column=5, ipady=5)
                maxCapacity = ttk.Label(frame, text = meat[3]).grid(row=i, column=7, ipady=5)
                capacityUtilisationLabel = ttk.Label(frame, text = currentCapacityUtil).grid(row=i, column=9, ipady=5)
                i += 2
